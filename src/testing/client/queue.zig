...
pub fn testQueueGroupDistribution(allocator: std.mem.Allocator) void {
    ...
    while (true) {
        const msg = sub1.nextWithTimeout(50) catch {
            break;
        };
        if (msg) |m| {
            m.deinit();
    ...
    while (true) {
        const msg = sub2.nextWithTimeout(50) catch {
            break;
        };
        if (msg) |m| {
            m.deinit();
    ...
    while (true) {
        const msg = sub3.nextWithTimeout(50) catch {
            break;
        };
        if (msg) |m| {
            m.deinit();
...
pub fn testQueueGroupMultipleClients(allocator: std.mem.Allocator) void {
    ...
    for (0..20) |_| {
        if (sub_a.nextWithTimeout(100) catch null) |m| {
            m.deinit();
            count_a += 1;
        }
    }
    for (0..20) |_| {
        if (sub_b.nextWithTimeout(100) catch null) |m| {
            m.deinit();
            count_b += 1;
        }
    }
...
pub fn testMultipleQueueGroups(allocator: std.mem.Allocator) void {
    ...
    var count: u32 = 0;
    if (sub_a.nextWithTimeout(500) catch null) |m| {
        m.deinit();
        count += 1;
    }
    if (sub_b.nextWithTimeout(500) catch null) |m| {
        m.deinit();
        count += 1;
    }
...
pub fn testQueueMemberJoinsMidStream(allocator: std.mem.Allocator) void {
    ...
    for (0..20) |_| {
        if (sub1.nextWithTimeout(100) catch null) |m| {
            m.deinit();
            count1 += 1;
        }
    }
    for (0..20) |_| {
        if (sub2.nextWithTimeout(100) catch null) |m| {
            m.deinit();
            count2 += 1;
        }
    }
...
pub fn testLargeQueueGroup(allocator: std.mem.Allocator) void {
    ...
    for (0..NUM_SUBS) |i| {
        if (subs[i]) |sub| {
            for (0..NUM_MSGS) |_| {
                if (sub.nextWithTimeout(50) catch null) |m| {
                    m.deinit();
                    total += 1;
                } else break;
            }
